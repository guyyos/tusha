...
def get_app_tabs():
    ...
    return html.Div(
                    [
                        ...
                        dbc.Tabs(
                            [
                                dbc.Tab(label="Explore", tab_id="tab-eda-parent", labelClassName="text-success font-weight-bold",
                                        activeLabelClassName="text-danger",children=[dbc.Tabs(
                                        [
                                            dbc.Tab(label="A", tab_id="tab-eda", labelClassName="text-success font-weight-bold",
                                                    activeLabelClassName="text-danger",style=tab_inner_style,children=eda_layout),
                                            html.Div(children=[],id='additional-eda-tabs'),
                                            dbc.Tab(label="+", tab_id="tab-eda-plus",
                                                    labelClassName="text-success font-weight-bold", 
                                                    activeLabelClassName="text-danger",style=tab_inner_style,children=[]),
                                        ],
                                        id="tabs-in",
                                        active_tab="tab-eda",
                                    )]),
                            ...
                            id="tabs",
                            active_tab="tab-eda-parent",
                        ),
                    ], className="mt-3"
                )


@app.callback(Output('additional-eda-tabs', 'children'),
              Input('tabs-in', 'active_tab'),
              State('additional-eda-tabs', 'children'))
def render_content(tab,additional_tabs):
    print(f'{tab} {additional_tabs}')
    if tab == 'tab-eda-plus':
        additional_tabs.append(dbc.Tab(label=f'{len(additional_tabs)}', tab_id=f'add_tab_{len(additional_tabs)}',
                                        labelClassName="text-success font-weight-bold", 
                                        activeLabelClassName="text-danger",style=tab_inner_style,children=[]))
        print(f'after adding:  {additional_tabs}')
